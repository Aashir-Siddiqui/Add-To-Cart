...
fetch('https://fakestoreapi.com/products')
    ...
    .then((data) => {
        ...
        let generateShop = (allCheckCategory = [], searchQuery = '') => {
            ...
            shop.innerHTML = data
                .filter((x) => {
                    const matchesCategory = allCheckCategory.length === 0 || allCheckCategory.includes(x.category);
        ...
        window.categoryFilter = () => {
            ...
            selectedCategories = checkData;
            generateShop(checkData, searchInput.value);
        };

        let searchInput = document.getElementById("search-input");
        searchInput.addEventListener('input', () => {
            generateShop(selectedCategories, searchInput.value);
        });
    })
    .catch((err) => {
        console.log(err);
